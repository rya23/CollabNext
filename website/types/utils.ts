import { Color, Side, EllipseLayer, Layer, LayerType, Point, XYWH, PathLayer, Camera } from './whiteboard';
...
export function findIntersectingLayerWithPoint(layerIds: string[], layers: Map<string, Layer>, point: Point) {
...
export function isHittingEllipse(layer: EllipseLayer, point: Point) {
    ...
    const result = Math.pow(point.x - cx, 2) / Math.pow(rx, 2) + Math.pow(point.y - cy, 2) / Math.pow(ry, 2);
