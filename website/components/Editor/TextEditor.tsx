...
import { FloatingComposer, FloatingThreads, useLiveblocksExtension, FloatingToolbar } from '@liveblocks/react-tiptap';
...
import { getFirestore, doc, setDoc, getDoc, collection, addDoc, query, orderBy, getDocs, Timestamp } from 'firebase/firestore';
...
export default function TextEditor({ fileId }: TextEditorProps) {
    return <ClientSideSuspense fallback={<DocumentSpinner />}>{() => <EditorWithStorage fileId={fileId} />}</ClientSideSuspense>;
}
...
function EditorWithStorage({ fileId }: TextEditorProps) {
    ...
    const fileIdFromParams = typeof params?.fileId === 'string' ? params.fileId : '';
        ...
        async function loadFromFirebase() {
            try {
                ...
                if (fileDoc.exists()) {
                    ...
                    console.log('Firebase content loaded, length:', fileData.content?.length);
    ...
    }, [firebaseLoaded, hasStorage, storageContent, localContent, initStorage, storageInitialized]);
    ...
    useEffect(() => {
        if (editor && localContent && editor.getHTML() !== localContent) {
            console.log('Updating editor with local content, length:', localContent.length);
    ...
    return (
        <div className="h-full flex flex-col">
            <div className="h-full">
                <div className={styles.container}>
                    ...
                    <div className={styles.editorPanel}>
                        ...
                        {editor && <AIChat editor={editor} isOpen={isAIChatOpen} onClose={() => setIsAIChatOpen(false)} />}
                        {editor && <Autocomplete editor={editor} />}
                        {editor && (
                            <StoryAnalysis editor={editor} isVisible={showAnalysis} onClose={() => setShowAnalysis(false)} />
                        )}
                    </div>
                </div>
            </div>
            {isSaving && <div className="py-1 px-3 text-xs text-white/50 bg-[#111111] border-t border-[#1F1F1F]">Saving...</div>}
            ...
            {editor && <ContentSummarizer editor={editor} isVisible={showSummarizer} onClose={() => setShowSummarizer(false)} />}
        </div>
    );
}
